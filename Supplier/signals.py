...
def cascade_material_archive_to_product(sender, instance, **kwargs):
    ...
    Material 'inactive' → archives linked Product.
    Material 'active' or 'on_hold' → restores linked Product."""
    from Product.models import Product

    if instance.status == 'inactive':
        Product.all_objects.filter(
            material=instance, is_active=True
        ).update(is_active=False)
    else:
        # 'active' or 'on_hold' both mean "in use" → restore the product
        Product.all_objects.filter(
            material=instance, is_active=False
